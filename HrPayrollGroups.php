...
<td>'.$myrow['currency'].'</td>
		...
		<td><a href="HrPayrollGroups.php?SelectedGroupName='.$myrow['payrollgroup_id'].'">' . _('Edit') . '</a></td>
		<td><a href="HrPayrollGroups.php?SelectedGroupName='.$myrow['payrollgroup_id'].'&amp;delete=yes" onclick=\'return confirm("' . _('Are you sure you wish to delete this Payroll group?') . '");\'>' . _('Delete') . '</a></td>
		</tr>';
...
if (isset($SelectedGroupName)) {
	...
	echo '<div class="centre"><br /><a href="' . htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8') . '">' . _('Show All Groups Defined') . '</a></div>';
}
if (! isset($_GET['delete'])) {
	...
	if ( isset($SelectedGroupName) AND $SelectedGroupName!='' ) {
		...
		echo '<tr>
				<td>' . _('GroupName ID') . ': ' . $_POST['payrollgroup_id'] . '</td>
			</tr>';
	} else 	{
	...
	echo '<tr>
			<td>' . _('Pay Group  Name') . ':</td>
			<td><input type="text" name="GroupName"  required="required" title="' . _('The Pay Group Name is required') . '" value="' . $_POST['GroupName'] . '" /></td>
		</tr>';

		// General Legder  input.
		echo '<tr><td><label for="PaymentFrequency">' . _('Payment Frequency') .
			':</label></td><td><select id="PaymentFrequency" name="PaymentFrequency" >';
		...
		echo '</select> </td></tr>
		<tr>
				<td>' . _('Generation Date') . ':</td>
				<td><input type="text" name="GenerationDate"  required="required" title="' . _('The Generation Date is required') . '" value="' . $_POST['GenerationDate'] . '" /></td>
			</tr>
			...
			<tr><td><label for="EnableLop">' . _('Enable LOP') .
			  ':</label></td>
			  <td><input type="radio"';
			...
			echo'
			  ...
			  type="radio" name="EnableLop"  id ="EnableLop" value="0"> No
				...
				<tr>
						<td>' . _('LOP Value') . ':</td>
						<td><input type="text" id="LopValue" name="LopValue"  value="' . $_POST['LopValue'] . '" /></td>
					</tr>

				';
				// General Legder  input.

					echo '<tr><td><label for="PayrollCategories">' . _('Payroll Categories') .
						':</label></td><td><select id="PayrollCategories" name="PayrollCategories[]" multiple="multiple" >';
...
$Payrollcats[$Row['payroll_category_id']] = $Row['payroll_category_id'];
					...
					echo '</select> </td></tr>';
					echo '<tr><td>' . _('Currency For Payroll Group') .'</td><td><select name="Currency">';
