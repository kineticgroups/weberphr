...
echo '<p class="page_title_text"><img src="'.$RootPath.'/css/'.$Theme.'/images/maintenance.png" title="' . _('employees') .
	'" alt="" />' . _('Employee Advance ') . '</p>';
...
<td>%s</td>
		...
		<td><a href="%sSelectedLoan=%s">' . _('%s') . '</a></td>
		<td><a href="%sSelectedLoan=%s&amp;delete=yes" onclick=\'return confirm("' . _('Are you sure you wish to delete this loan ?') . '");\'>' . _('Delete') . '</a></td>
		</tr>',
...
if (isset($SelectedLoan)) {
	...
	echo '<div class="centre"><br /><a href="' . htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8') . '">' . _('Show All Loans') . '</a></div>';
}
...
$_POST['GeneralLegder']  = $myrow['gl_posting_account'];
	...
	echo '<tr>
			<td>' . _('Employee') . ':</td>
			<td><select ' . (in_array('EmpID',$Errors) ?  'class="inputerror"' : '' ) .' name="EmpID"><option value="0">Select employee</option>';
	...
	echo '</select></td></tr>';
		...
		echo '<tr><td><label for="LoanType">' . _('Loan Type') .
			':</label></td><td><select id="PaymentFrequency" name="LoanType" >';
		...
		<tr>
				<td>' . _('Loan Amount') . ':</td>
				<td><input ' . (in_array('LoanAmount',$Errors) ?  'class="inputerror"' : '' ) .' id="LoanAmount" type="text" name="LoanAmount"  required="required" title="' . _('The Loan Amount') . '" value="' . $_POST['LoanAmount'] . '" /></td>
			</tr>';
			echo '<tr>
					<td>' . _('No of Salaries to Deduct from') . ': (Installments)</td>
					<td><select id="Installments" name="Installments">';
					for($i=1;$i<13;$i++)
					{
	...
	echo '</select></td>
			...
			<tr>
					<td>' . _('Installment Amount') . ':</td>
					<td><input id="InstallmentAmount" readonly type="text" name="InstallmentAmount"  value="' . $_POST['InstallmentAmount'] . '" /></td>
				</tr>
			<tr><td><label for="Approve">' . _('Approve and Pay?') .
			  ':</label></td>
			  <td><input type="radio"';
