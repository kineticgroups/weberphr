...
echo '<form action="' . htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8') . '" method="post">
    ...
    echo ''. _('Payroll Group') . ':<select required="required" id="select-payrollgroup" name="PayrollGroupID"> <option value="">select payroll group</option>';
    ...
    while ($myrow=DB_fetch_array($resultPayrollGroups)){
      ...
      echo '</select> &nbsp;&nbsp'._('Payment Period') . ': <input type="text" name="PaymentPeriod" required="required"  id="datepicker" maxlength="10" size="10"  />&nbsp';
      echo '<input type="submit" name="GeneratePayroll" value="' . _('Generate ') . '" onclick="return confirm(\'', _('Are you sure you wish to generate payroll?'), '\');" />
        &nbsp;&nbsp;</td>
        </tr>
        </table>
        <br />
...
if(isset($GeneratedPayroll))
{
	...
	echo '<h3>Payroll Group'.$_POST['PayrollGroupID'].'</h3>';
	echo '<table>
...
if(isset($_POST['GeneratePayroll'])) {
			...
			if (DB_error_no() ==0) {
				...
				echo '<h3>Payroll Group - '.$PayrollGroupName.'</h3>';
				echo '<table>
								<tr><td>Pay period</td><td>Payment frequency</td><td>Payslips generated</td><td>Total Net Salary</td><td>Bulk actions</td></tr>
				...
				foreach($payroll_array as $index => $employee_payslip){
					...
					echo'	<td>'.number_format($employee_payslip['employee_other_deductions'],2).'</td>
						...
						//	echo	'<td class="noprint"><a href="HrEmployees.php?EmpID='. $employee_payslip['employee_id']. '">'. _('Edit'). '</a></td>';
								...
								{
		    					echo '<td class="noprint"><a href="HrPrintPayslip.php?PayslipNo='. $employee_payslip['payslip_id']. '">'. _('Print Payslip'). '</a></td>';
								}
					echo '</tr>';
				}
			}
