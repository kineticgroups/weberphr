...
echo '<p class="page_title_text"><img src="'.$RootPath.'/css/'.$Theme.'/images/user.png"  alt="" />' . _('Employees Leaves ') . '</p>';
...
echo '<a href="' . $RootPath . '/HrSelectLeave.php">' . _('Search For  Leaves') . '</a><br />' . "\n";
...
echo'</td>
		...
		{
		echo'<a href="HrLeaveApplications.php?SelectedName='.$myrow['employee_leave_id'].'&amp;delete=yes" onclick=\'return confirm("' . _('Are you sure you wish to delete this Category Name?') . '");\'>' . _('Delete') . '</a>';
		}
		echo'</td>
		</tr>';
...
if (isset($SelectedName)) {
	...
	echo '<div class="centre"><br /><a href="' . htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8') . '">' . _('Show All Leaves Defined') . '</a></div>';
}
...
$_POST['ApprovingManager']  = $myrow['leave_approving_manager'];
		...
		echo '<tr>
				<td>' . _('Leave ID') . ': ' . $_POST['employee_leave_id'] . '</td>
			</tr>';
	} else 	{
	...
	if (DB_Num_Rows($magagerfetch)>0){
		...
		echo '
		<tr><td><label for="EmployeeId">' . _('Employee') .
			':</label></td><td><select id="EmployeeId" name="EmployeeId" >';
	...
	echo'<tr><td>Employee</td><td><input type="text" value="'.$userrow['employee_id'].' - ' .$userrow['first_name'].' ' .$userrow['middle_name'].' '.$userrow['last_name'].'" name="Employee" size="50">
...
echo'	<tr><td><label for="LeaveType">' . _('Leave Type') .
		':</label></td><td><select id="LeaveType" name="LeaveType" >';
	...
	echo '</select> </td></tr>
			...
			<tr><td><label for="LeaveDuration">' . _('Leave duration') .
				':</label></td>
				<td><input type="radio"';
				if (!isset($SelectedName)) {
				 echo ' checked';}
...
echo'	<tr><td><label for="Approved1">' . _('Approved') .
					':</label></td>
					<td><input type="radio"';
				...
				echo'
					...
					<tr><td><label for="ViewManager">' . _('Viewed By Manager') .
						':</label></td>
						<td><input type="radio"';
				...
				<tr>
					<td>' . _('Managers Remarks') . ':</td>
					<td>
				<textarea name="Remarks"  >' . $_POST['Remarks'] . '</textarea><br
				>
				</tr>';
			}else{
	if (!isset($SelectedName)) {
...
';
	...
	if (isset($SelectedName)) {
						echo'<tr><td>' . _('Approving Manager') . ':</td>
						<td><input type="text" name="ApprovingManager"   value="' .$Employeelist[$_POST['ApprovingManager']]  . '" size="40" /></td>
					</tr>';
				}
		echo'</table>
		<br />
...
if (isset($SelectedName)) {
		if($_POST['Approved']==0)
		{
		echo'<input type="submit" name="submit" value="' . _('Accept') . '" /></a>';
		}
	}else{
