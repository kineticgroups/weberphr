...
//    if (!isset($EN) or ($EN=='')){
    ...
    echo  _('Department') . ':<select name="Department"> <option value="">select department</option>';
    ...
    while ($myrowdept=DB_fetch_array($resultDepartments)){
        ...
        $months_string = "";
         for($m=1; $m<=12; ++$m){
  		...
  		echo '</select> &nbsp;&nbsp'._('Attendance Period') . ':
        <select name="currentMonth">'.$months_string.'</select>&nbsp';
      echo '<input type="submit" name="SearchAttendance" value="' . _('Search') . '" />
  			&nbsp;&nbsp;</td>
  			</tr>
  			</table>
  			<br />
...
echo'<div style="overflow-x:auto;margin:0px 40px">';
    echo '<table class="selection1">';
    echo '<tr>
              <th>Employee</th>';
...
foreach($days_in_month_array as $day)
{
    ...
    echo '<td><input'._(in_array(($month_date), $AllAbsentDays) ? ' checked':'').' type="checkbox" name="AbsentDate[]" value="'.date('Y-m-d',mktime(0, 0, 0, $month_selected, $day, $current_year)).'">';
		...
		echo  '<select id="LeaveType" name="LeaveType[]">';
$AllLeaveTypes = array();
    ...
    echo '</select></td>';
      ...
      echo'    <td><input type="submit" name="SearchAttendance" value="' . _('Save') . '" /></td>
